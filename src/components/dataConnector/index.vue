...
export default {
  ...
  methods: {
    ...
    onFinish(values) {
      ...
      delete values.name;
      componentAdd({...this.spContext, componentType: 'DataBase', name, instanceInfo: JSON.stringify(values)}).then(
        res => {
          if (res.data) {
...
<template>
<a-tabs v-model:activeKey="activeKey" type="card" @change="onTabChange" :tabBarGutter="8" :tabBarStyle="{margin: 0}">
  <a-tab-pane key="add" tab="新增">
    ...
    >
      ...
      <a-form-item class="footer" :wrapper-col="{offset: 4, span: 19}">
